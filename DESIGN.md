...
### Grind Converter
- **Purpose:** Translate a setting on one grinder into the equivalent setting on another without inventing data. Every result is anchored to a brew method for which both grinder guides cite a numeric range.
- **Directory entry:** The converter appears above the grinder records as a compact surfaced utility strip, with a dedicated swap mark and explicit action label. Its denser monospace hierarchy distinguishes a tool from the larger serif model rows without turning it into a marketing card.
- **Form:** A single semi-transparent panel (`oklch(1 0 0 / 0.2)`, `1px solid wisteria-rule`, 10px radius) groups the source grinder and setting, followed by a swap pill and the target grinder. The setting includes the source chart scale as persistent help. Its text-entry spinbutton uses explicit minus/plus controls so indexed subdivisions carry correctly across whole labels (for example `5.2 → 6`); selects and the setting shell share the same 44px height and panel border language. Labels use the serif italic `.label` caption voice. The route stacks at ≤820px and the source controls stack at ≤560px.
- **Results:** Rule-separated rows, never cards. A compact functional heading and conversion summary lead into each method row: serif method name, prominent monospaced target setting, explicitly labeled source and target published bands, then a scale line showing the target band and a Deep Violet marker bracketed by the target's scale endpoints.
- **Honesty rules:** A result always names the method it is anchored to. Extrapolated results preserve their position beyond the nearest shared range and are labeled as such; unrelated grinder pairs say so instead of guessing. Micron spans are never used to synthesize a conversion. Inputs and results are quantized using each grinder's declared `setting_subdivisions`, never inferred from whether its unit label says “clicks” or “numbers.”
- **Feedback and accessibility:** Invalid positions and out-of-scale values appear beside the setting field; pair-level states remain in the result region. One visually hidden polite live region announces concise updates instead of re-announcing the result list. The converter suppresses ambient ASCII motion so attention stays on the controls and result.
- **i18n:** Both languages are rendered as `.lang-en` / `.lang-zh` spans so the existing body-class toggle drives visibility; visible region labels and help/error copy switch with the page language while grinder names and numerals stay language-neutral.
